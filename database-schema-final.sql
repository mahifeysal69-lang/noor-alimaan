-- Noor Al Imaan Database Schema (Final Version)
-- This file creates tables for user registrations and feedbacks
-- Handles existing policies gracefully and avoids CONCURRENTLY errors
...
-- Indexes for user_registrations (without CONCURRENTLY to avoid transaction errors)
CREATE INDEX IF NOT EXISTS idx_user_registrations_created_at ON user_registrations(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_user_registrations_city_oromo ON user_registrations(city_oromo);
CREATE INDEX IF NOT EXISTS idx_user_registrations_city_english ON user_registrations(city_english);
CREATE INDEX IF NOT EXISTS idx_user_registrations_mosque_oromo ON user_registrations(mosque_oromo);
CREATE INDEX IF NOT EXISTS idx_user_registrations_mosque_english ON user_registrations(mosque_english);
CREATE INDEX IF NOT EXISTS idx_user_registrations_phone_oromo ON user_registrations(phone_oromo);
CREATE INDEX IF NOT EXISTS idx_user_registrations_phone_english ON user_registrations(phone_english);

-- Indexes for user_feedbacks (without CONCURRENTLY to avoid transaction errors)
...
CREATE INDEX IF NOT EXISTS idx_user_feedbacks_category ON user_feedbacks(category);
CREATE INDEX IF NOT EXISTS idx_user_feedbacks_urgency ON user_feedbacks(urgency_level);
CREATE INDEX IF NOT EXISTS idx_user_feedbacks_user_reg_id ON user_feedbacks(user_registration_id);
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow public insert registrations' AND tablename = 'user_registrations') THEN
        EXECUTE 'CREATE POLICY "Allow public insert registrations" ON user_registrations FOR INSERT WITH CHECK (true)';
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow public read registrations' AND tablename = 'user_registrations') THEN
        EXECUTE 'create POLICY "Allow public read registrations" ON user_registrations FOR SELECT USING (true)';
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow authenticated update registrations' AND tablename = 'user_registrations') THEN
        EXECUTE 'create POLICY "Allow authenticated update registrations" ON user_registrations FOR UPDATE USING (auth.role() = ''authenticated'')';
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow authenticated delete registrations' AND tablename = 'user_registrations') THEN
        EXECUTE 'create POLICY "Allow authenticated delete registrations" ON user_registrations FOR DELETE USING (auth.role() = ''authenticated'')';
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow public insert feedbacks' AND tablename = 'user_feedbacks') THEN
        EXECUTE 'create POLICY "Allow public insert feedbacks" ON user_feedbacks FOR INSERT WITH CHECK (true)';
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow public read feedbacks' AND tablename = 'user_feedbacks') THEN
        EXECUTE 'create POLICY "Allow public read feedbacks" ON user_feedbacks FOR SELECT USING (true)';
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow authenticated update feedbacks' AND tablename = 'user_feedbacks') THEN
        EXECUTE 'create POLICY "Allow authenticated update feedbacks" ON user_feedbacks FOR UPDATE USING (auth.role() = ''authenticated'')';
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow authenticated delete feedbacks' AND tablename = 'user_feedbacks') THEN
        EXECUTE 'create POLICY "Allow authenticated delete feedbacks" ON user_feedbacks FOR DELETE USING (auth.role() = ''authenticated'')';
    END IF;
END $$;
...
INSERT INTO user_registrations (
    ...
    'Fatima Abdi', 32, 'Dirre Dhawaa', 'Dubartii', 'Ganda B, Wereda 07', 'Masjid Al-Taqwa', '+251 92 234 5678',
    'Fatima Abdi', 32, 'Adama', 'Female', 'District 07', 'Al-Taqwa Mosque', '+251 92 234 5678',
...
-- No CONCURRENTLY errors expected
