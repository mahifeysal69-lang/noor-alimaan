-- Improved Database Schema for Noor Al Imaan Interactive Features
-- This addresses gaps identified in the assessment

-- Comments Table with admin review field
...
-- Donations Table with improved tracking
...
-- Materials Table with custom material field
...
CREATE TABLE IF NOT EXISTS user_sessions (
    ...
    user_registration_id UUID REFERENCES user_registrations(id) ON DELETE SET NULL
);
...
CREATE OR REPLACE FUNCTION link_session_to_registration(p_session_id TEXT, p_registration_id UUID)
RETURNS VOID AS $$
BEGIN
